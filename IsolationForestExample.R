...
ggplot(iso_DF_ordered, 
       aes(x=c(1:nrow(iso_DF_ordered)), 
           y=iso_DF_ordered$mean_length)) +
    geom_point(size=2, shape=1) + 
  ylab("Mean path length for isolation") + 
  xlab("Cases") + 
  geom_hline(yintercept = anomaly_threshold) + 
